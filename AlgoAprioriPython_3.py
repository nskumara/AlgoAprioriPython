    ...
    def __repr__(self):
        ...
        for size, itemsets in self.itemsets.items():
            ...
            for itemset in itemsets:
                result += f"{itemset} (Support: {itemset.support})\n"
        return result
...
class AlgoApriori:
    ...
    def save_itemset(self, itemset):
        ...
        if self.writer is not None:
            self.writer.write(str(itemset.itemset) + " #SUP: " + str(itemset.support) + "\n")
        else:
...
# Example usage:
algo = AlgoApriori()
result = algo.run_algorithm(0.4, "your_input_file.txt", "your_output_file.txt")
algo.print_stats()
print(result)
